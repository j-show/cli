...
本包默认内置并自动注册了一组命令（开箱即用）：

- `release`：Monorepo 发包向导（当前版本 bump / 发布逻辑为占位实现）
- `backup`：备份工作区包到输出目录（可选清理输出中的 `.git`）

实现位置：`src/built-in/commands/`。
...
- `flagValue?: boolean` - 该选项是否需要值（为 `true` 时会生成 `--name <value>`）
